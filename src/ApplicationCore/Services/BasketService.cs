...
namespace ApplicationCore.Services
{
    public class BasketService : IBasketService
    {
        ...
        public async Task TransferBasketAsync(string anonymousId, string userId)
        {
            ...
            var sourceBasket = await _basketRepo.FirstOrDefaultAsync(specAnon);
            if (sourceBasket == null) return;

            var specUser = new BasketWithItemsSpecification(userId);
            var destinationBasket = await _basketRepo.FirstOrDefaultAsync(specUser);

            if (destinationBasket == null)
            {
                destinationBasket = new Basket() { BuyerId = userId };
                await _basketRepo.AddAsync(destinationBasket);
            }

            foreach (BasketItem sourceItem in sourceBasket.Items)
            {
                BasketItem targetItem = destinationBasket.Items.FirstOrDefault(x => x.ProductId == sourceItem.ProductId);
                if (targetItem != null)
                    targetItem.Quantity += sourceItem.Quantity;
                else
                    destinationBasket.Items.Add(new BasketItem() { ProductId = sourceItem.ProductId, Quantity = sourceItem.Quantity });
            }
